...
class ContrastiveSampler(Sampler):
    ...
    #def load_generate_ADVneg(self):

    def load_hard_negatives(self):
        """Load negatives of type `Image` and `Question` """
        negs_path = "/data1/MitraTj/Proj/Contrastive/concat-vqa/data-release/negative-files/fil_{}_question_negs.pkl".format(
            self.split
        )
    ...
    def read_annotations(self):
        ...
        ann_path = [
            "/data1/MitraTj/Proj/Contrastive/concat-vqa/data-release/splits/v2_mscoco_train2014_annotations.json",
            "/data1/MitraTj/Proj/Contrastive/concat-vqa/data-release/splits/v2_mscoco_val2014_annotations.json",
        ]
        ann_data = (
    ...
    @staticmethod
    def get_hard_negative(negative_list, batch_bins, entry_map, question_rephrase_dict):     ##def _get_hard_batch(self, batch, scores, sample_from='t'):
        ...
        for batch_inds, batch_bins in tqdm(
            ...
            while True:
                neg_choice = np.random.choice(
                    ["image_neg","random", "question_neg"], p=neg_type_weights
                )
                passed = False
...
class TokenBucketSampler(Sampler):
    def __init__(self, data_source, task_cfg, droplast=False):    #batch_size    bucket_size:8192               
        #self.data_source = data_source
        self.batch_size = task_cfg["batch_size"]
        self._max_tok = task_cfg["batch_size"]
        self._droplast = droplast
        size_multiple = 8
        self._size_mul = size_multiple
        bucket_size = 8192
        self._bucket_size = bucket_size
        self._lens = data_source.entries
    ...
    def __iter__(self):
        ...
        random.shuffle(ids)       ##np.random.shuffle(array[start_idx:end_idx])
        ...
        for bucket in buckets:
            ...
            if not self._droplast and batch_indices:
                batches.append(batch_indices)
        random.shuffle(batches)        ##ContrastiveSampler.shuffle(self.re_bins, 0, len(self.re_bins))
        return iter(batches)
    ...
    def __len__(self):
        return len(self.data_source)
##################################################################
## https://github.com/ChenRocks/UNITER/blob/master/data/itm.py
#################################################################
