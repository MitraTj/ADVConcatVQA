...
class ScaledSupConLoss(torch.nn.Module):
    ...
    def __init__(
        self,
        temperature=0.07,     ##change to 0.5
        contrast_mode="all",
    ...
    def forward(self, batch_dict):
        ...
        labels = batch_dict[0]["target"].argmax(dim=-1).view(-1, 1)   ##labels = torch.arange(batch_size, device=input_ids.device)

        # samples without an answer cannot work as anchor points
        mask_samples = (batch_dict[0]["target"].sum(dim=-1) != 0).int()    ##pos_sim = cos(avg_doc, avg_pos_dec).unsqueeze(-1)
        ...
        batch_size = features.shape[0]     ##batch_size = input_ids.size(0)
        if labels is not None and pos_mask is not None:
            raise ValueError("Cannot define both `labels` and `mask`")
        elif labels is None and pos_mask is None:
            pos_mask = torch.eye(batch_size, dtype=torch.float32).to(device)    ##identity = torch.eye(batch_size, device=input_ids.device)
        ...
        pos_mask = pos_mask * mask_samples      ##pos_sim = identity * pos_sim
        ...
        scl_mask_rescale_factor = registry.scl_mask_rescale_factor   #20
...
LossMap = {
    ...
    "criterion_dis": torch.nn.MultiLabelMarginLoss(reduction="mean"),
...
def ce_loss(batch_dict, device, val_run=False, revqa_eval=False, split="revqa"):
    if len(batch_dict) == 2 and not val_run:
        ...
        vil_targets = batch_dict["target"]   ##[210, 3129]

    vl_loss_c = LossMap["BCEWithLogitLoss"](vil_preds, vil_targets)    ##binary_cross_entropy_with_logits
    vl_loss_c = vl_loss_c.mean() * vil_targets.size(1)
    batch_scores = compute_score_with_logits(vil_preds, vil_targets, device)
    batch_score = batch_scores.sum() / len(vil_preds)
    ### added
    #import pdb; pdb. set_trace()
    ##vil_preds_d = vil_preds.max(1)[0]    #vil_preds_dd = vil_preds.max(1)[1]
    
    ##vil_targets_d = torch.zeros(vil_preds_d.size(-1), vil_preds_d.size(0))
    vil_targets_d = torch.zeros(vil_preds.size(0), vil_preds.size(1)).to(device)    ##[210, 210]
    vil_targets_d.fill_(-1)

    vl_loss_d = LossMap["criterion_dis"](vil_preds, vil_targets_d.long()) 
    vl_loss_d = vl_loss_d.mean() * vil_targets_d.size(1)
    
    vl_loss = vl_loss_c + (10 * vl_loss_d)
