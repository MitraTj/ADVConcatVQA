...
def load_qa(name, sort=True, use_filter=False, set_dict=False):
    split_path_dict = {
        "train_aug": [
            "/data1/MitraTj/Proj/Contrastive/concat-vqa/data-release/splits/questions_train_aug.pkl",
            "/data1/MitraTj/Proj/Contrastive/concat-vqa/data-release/splits/ans_train_aug.pkl",
            "train",
        ],
        "train": [
            "/data1/MitraTj/Proj/Contrastive/concat-vqa/data-release/splits/v2_OpenEnded_mscoco_train2014_questions.json",
            "/data1/MitraTj/Proj/Contrastive/concat-vqa/data-release/splits/train_target.pkl",
            "train",
        ],
        "val": [
            "/data1/MitraTj/Proj/Contrastive/concat-vqa/data-release/splits/v2_OpenEnded_mscoco_val2014_questions.json",
            "/data1/MitraTj/Proj/Contrastive/concat-vqa/data-release/splits/val_target.pkl",
            "val",
        ],
        "val_aug": [
            "/data1/MitraTj/Proj/Contrastive/concat-vqa/data-release/splits/questions_val_aug.pkl",
            "/data1/MitraTj/Proj/Contrastive/concat-vqa/data-release/splits/ans_val_aug.pkl",
            "val",
        ],
        "test": [
            "/data1/MitraTj/Proj/Contrastive/concat-vqa/data-release/splits/v2_OpenEnded_mscoco_test2015_questions.json",
            "",
            "test",
        ],
        "trainval_aug": [
            "/data1/MitraTj/Proj/Contrastive/concat-vqa/data-release/splits/questions_trainval_aug.pkl",
            "/data1/MitraTj/Proj/Contrastive/concat-vqa/data-release/splits/ans_trainval_aug.pkl",
            "trainval",
        ],
        "revqa": [
            "/data1/MitraTj/Proj/Contrastive/concat-vqa/data-release/splits/revqa_total_proc.pkl",
            "/data1/MitraTj/Proj/Contrastive/concat-vqa/data-release/splits/revqa_total_proc_target.pkl",
            "revqa",
        ],
    }
...
def load_entries(name):
    ...
    if name == "train" or name == "val":
        ...
        dump_path = "/data1/MitraTj/Proj/Contrastive/concat-vqa/data-release/splits/non_overlap_ids.npy"
...
class VQAClassificationDataset(Dataset):
    def __init__(
        ...
        self.ans2label = cPickle.load(
            open("/data1/MitraTj/Proj/Contrastive/concat-vqa/data-release/splits/trainval_ans2label.pkl", "rb")
        )
        self.label2ans = cPickle.load(
            open("/data1/MitraTj/Proj/Contrastive/concat-vqa/data-release/splits/trainval_label2ans.pkl", "rb")
        )
        # attach to registry
        ...
        #self._padding_index=tokenizer.pad_token_id
        self._padding_index = padding_index
        self.extra_args = extra_args

        self.entries = load_entries(split)
        # convert questions to tokens, create masks, segment_ids
        self.tokenize(self._max_seq_length)        ##txt_db = TxtTokLmdb(txt_path, opts.max_txt_len)
        self.tensorize()
    ...
    def tensorize(self):
        for entry in tqdm(self.entries, desc="Tensorizing..."):
            ...
            if "test" not in self.split:
                answer = entry["answer"]           ##target = torch.zeros(num_answers)
                labels = np.array(answer["labels"])
                scores = np.array(answer["scores"], dtype=np.float32)
                if len(labels):
                    labels = torch.from_numpy(labels)
                    scores = torch.from_numpy(scores)
                    entry["answer"]["labels"] = labels     ##labels = example['target']['labels']
                    entry["answer"]["scores"] = scores
                else:
                    entry["answer"]["labels"] = None
                    entry["answer"]["scores"] = None

    def _get_img_feat(self, fname):                 ##VqaDataset(DetectFeatTxtTokDataset)
        img_feat, bb = self.img_db[fname]
        img_bb = torch.cat([bb, bb[:, 4:5]*bb[:, 5:]], dim=-1)
        num_bb = img_feat.size(0)
        return img_feat, img_bb, num_bb

    def __getitem__(self, index):    ##def __getitem__(self, i):
        ...
        features, num_boxes, boxes, _ = self._image_features_reader[image_id]    ##img_feat, img_pos_feat, num_bb = self._get_img_feat(
        ...
        image_mask = torch.tensor(image_mask).long()               ##img_mask
        ...
        if "test" not in self.split:
            answer = entry["answer"]     ##target = torch.zeros(num_answers)
            labels = answer["labels"]    ##labels = example['target']['labels']
            scores = answer["scores"]    ##
            if labels is not None:
                target.scatter_(0, labels, scores)   ## target.scatter_(0, torch.tensor(labels), torch.tensor(scores))

        item_dict.update(      ##batch
            {
                "input_imgs": features,
                ...
                "target": target,
                "mask": torch.tensor(1),     ##attn_masks = torch.ones(len(input_ids) + num_bb, dtype=torch.long)
            }
        )
