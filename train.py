...
from mmtadvorg.task_utils import get_batch
#from mmtadvorg.losses import compute_score_with_logits
from tools.registry import registry
...
def get_config():
    ...
    ####
    parser.add_argument('--checkpoint', default='', type=str, help='path to pretrained model')
...
def set_device_folder(task_cfg, args):
    if torch.cuda.is_available():
        ...
        multi_gpu = torch.cuda.device_count() == 0,1,2 #> 1
        #multi_gpu = torch.cuda.device_count() == 0,1,2,3
    else:
...
def main():                                  ##def main(opts):
    """ Trains a model and evaluates it."""
    task_cfg, args = get_config()

    from mmtadvorg.mmt import MMT, BertConfig
    from mmtadvorg.task_utils import (
        clip_gradients,
        forward_train,
        pgd_attack,
        pgd_attack_opt,
    ...
    # load datasets
    dataloaders = load_dataset(task_cfg)     ##train_dataloader
    ...
    model = MMT(mmt_config, text_bert_config)          ##model = UniterForVisualQuestionAnswering.from_pretrained(opts.model_config, checkpoint,img_dim=IMG_DIM, num_answer=len(ans2label))
    ...
    for state in optimizer.state.values():
        for k, v in state.items():
            if torch.is_tensor(v):
                state[k] = v.cuda()
    if multi_gpu:
        model = torch.nn.DataParallel(model)    #torch.utils.data.Dataloader
        #model = torch.nn.DataParallel(model).cuda()
    #if args.checkpoint != "":
    #    logger.info(
    #        f"Dumping Evaluation results at: {os.path.dirname(args.checkpoint)}")
    # store best values
    ...
    # train loop
    #num_iters = len(dataloaders["train_adv"] if registry.alt_train else dataloaders["train_ce"])  #40344, 7684, 
    num_iters = len(dataloaders["train_scl"] if registry.alt_train else dataloaders["train_ce"])   #17930
    #dataloader = dataloaders["train_adv"] if registry.alt_train else dataloaders["train_ce"]
    model.train()
    
    #########################################################################################
    # Begin training on train/val or trainval/test
    #########################################################################################
    for epochId in tqdm(range(start_epoch, task_cfg["num_epoch"]), desc="Epoch"):           ##for step, batch in enumerate(train_dataloader):
        for step in tqdm(range(num_iters), desc="Iters"):                                   ##step, batch in enumerate(train_dataloader):
            ...
            #if registry.alt_train and iter_id % registry.ce_freq == 2:  #0  #0, 5, 8, 13, 17
            #    train_type = "scl"
            if registry.alt_train and iter_id % registry.ce_freq == 3: #1 #1, 4, 9, 12, 16
                train_type = "adv"
            else:
                train_type = "ce"  # "adv"     #2,3, 6, 7, 10, 11, 14, 15,18, 19
            ...
            if train_type == "ce" or train_type == "adv" or (not registry.alt_train):    ##
...
def evaluate_rephrasings(dataloaders, model, device):
    ...
    from mmtadvorg.task_utils import forward_eval
...
def run_evaluation(          ##def validate(
    dataloaders,
    device,
    model,
):
